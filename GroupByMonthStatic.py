...
globbed_files_static = glob('./grouped_data_static/*.csv')  # creates a list of all csv files


for file in globbed_files_static:
    ...
    try:
        ...
        os.makedirs('./grouped_data_static_by_month', exist_ok=True)
        for name, group in df_grouped_year:
            os.makedirs(f'/home/charalambos/Documents/Projects/ADAPTATION/grouped_data_static_by_month/{str(name)}', exist_ok=True)
            time_now = pd.Timestamp.now()
            ...
            for month, month_group in grouped_by_month:
                file_path = f'/home/charalambos/Documents/Projects/ADAPTATION/grouped_data_static_by_month/{str(name)}/{str(month)}.csv'
                time_now = pd.Timestamp.now()
        ...
        os.makedirs('./grouped_data_static/done', exist_ok=True)
        shutil.move(file, './grouped_data_static/done')
